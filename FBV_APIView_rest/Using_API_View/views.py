...
def category_av_list(request):
    if request.method == "GET":
        ...
        category_av_serializer = CategoryAVSerializer(category_av , many=True) # converts cmoplext objects(query set) into json
        return Response(category_av_serializer.data,status=status.HTTP_200_OK)
    elif request.method == "POST":
        category_av_serializer = CategoryAVSerializer(data=request.data) #this serializer converts JSON request into DB readable object.
        ...
        return Response(category_av_serializer.errors,status=status.HTTP_404_NO_CONTENT)
